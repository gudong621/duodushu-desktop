...
var oaldpeConfig = {
    ...
    collapseControl: {
        ...
        // 【配置项2：默认展开折叠块】
        unfoldUnbox: {
            selectedValue: false, // 默认折叠（用户手动点击展开）
            title: '默认展开折叠块',
            type: 'checkbox',
...
function main() {
    ...
    oaldpeInit.$allContainers.each(function () {
        ...
        function setupShcutFold() {
            $oaldpe.find('h2.shcut').each(function () {
                const $shcut = $(this);
                // 防止重复绑定事件（使用 DOM 属性而非 jQuery data 缓存）
                if (!$shcut.attr('data-oaldpe-bound')) {
                    $shcut.attr('data-oaldpe-bound', 'true');
                    $shcut.on('click', function (event) {
                        event.stopPropagation();
        ...
        function setupSenseFold() {
            senseMapping.forEach(({ $iteration, $senseExpand }) => {
                ...
                // 防止重复绑定事件（使用 DOM 属性而非 jQuery data 缓存）
                if (!$iteration.attr('data-oaldpe-bound')) {
                    $iteration.attr('data-oaldpe-bound', 'true');
                ...
                // 只在首次初始化时设置初始状态（使用 DOM 属性）
                if (!$iteration.attr('data-oaldpe-initialized')) {
                    $iteration.attr('data-oaldpe-initialized', 'true');
            ...
            // 只在首次初始化时设置 concise 属性（使用 DOM 属性）
            if (!$oaldpe.attr('data-oaldpe-concise-initialized')) {
                $oaldpe.attr('data-oaldpe-concise-initialized', 'true');
                if (!oaldpeConfig.collapseControl.unfoldSense.selectedValue) {
            ...
            $oaldpe.find('.idm, .pv').each(function () {
                const $heading = $(this);
                // 防止重复绑定事件（使用 DOM 属性）
                if (!$heading.attr('data-oaldpe-bound')) {
                    $heading.attr('data-oaldpe-bound', 'true');
        ...
        function setupUnboxFold() {
            $oaldpe.find('.collapse .unbox .box_title').each(function () {
                ...
                // 防止重复绑定事件（使用 DOM 属性）
                if (!$boxTitle.attr('data-oaldpe-bound')) {
                    $boxTitle.attr('data-oaldpe-bound', 'true');
                    $boxTitle.on('click', function (event) {
                        event.stopPropagation();

                        // 强制展开/折叠逻辑（确保工作）
                        if ($unbox.hasClass('is-active')) {
                            // 收起：设置display:block后slideUp
                            $content.css('display', 'block').slideUp(300);
                        } else {
                            // 展开：先设置display:block，再slideDown
                ...
                // 只在首次初始化时设置初始状态（使用 DOM 属性）
                if (!$unbox.attr('data-oaldpe-initialized')) {
                    $unbox.attr('data-oaldpe-initialized', 'true');

                    // 显式设置初始状态（移除依赖CSS默认）
                    $content.css('display', 'none');
                    $unbox.removeClass('is-active');

                    // 判断条件并相应设置
                    const shouldUnboxExpand = oaldpeConfig.collapseControl.unfoldUnbox.selectedValue ||
                        oaldpeConfig.collapseControl.autoUnfoldUnbox.selectedValue[$unbox.attr('unbox')]?.value;

                    if (shouldUnboxExpand) {
        ...
        function setupPhraseSections() {
            ...
            $oaldpe.find('.idioms, .phrasal_verb_links').each(function () {
                ...
                // 只在首次初始化时设置初始状态（使用 DOM 属性）
                if (!$section.attr('data-oaldpe-initialized')) {
                    $section.attr('data-oaldpe-initialized', 'true');
                ...
                // 防止重复绑定跳转链接事件
                if (!$jumpLink.data('oaldpe-bound')) {
                    $jumpLink.data('oaldpe-bound', true);
                ...
                // 防止重复绑定返回链接事件
                if (!$backLink.data('oaldpe-bound')) {
                    $backLink.data('oaldpe-bound', true);
