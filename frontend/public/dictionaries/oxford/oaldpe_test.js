...
function main() {
    ...
    oaldpeInit.$allContainers.each(function () {
        ...
        function setupExamplePronTTS() {
            ...
            console.log('[oaldpe] setupExamplePronTTS: Starting to add TTS audio to ALL examples');

            // 获取所有例句（不管结构如何）
            const $allExamples = $oaldpe.find('li').has('.x, .unx, span.x, span.unx');
            console.log('[oaldpe] setupExamplePronTTS: Found examples:', $allExamples.length);

            $allExamples.each(function (index) {
                const $example = $(this);
                
                // 获取例句文本
                let exampleText = '';
                const $x = $example.find('.x, span.x, .unx, span.unx').first();
                if ($x.length > 0) {
                    exampleText = $x.clone()
                        .find('gloss, chn, script, xt, .oald, .ai, .cl').remove().end().text()
                        .replace(/somebody\/something/g, 'somebody or something')
                        .replace(/&/g, 'and').replace(/\u200B/g, '')
                        .trim();
                } else {
                    // 如果没有找到 .x 或 .unx，使用 li 的文本
                    exampleText = $example.clone()
                        .find('gloss, chn, script, xt, .oald, .ai, .cl').remove().end().text()
                        .trim();
                }

                if (!exampleText) {
                    console.log('[oaldpe] Example', index, 'has no text');
                    return;
                }

                // 创建容器元素（用于插入音频按钮）
                const audioContainer = $('<span></span>').addClass('audio-buttons-wrapper');

                // 创建英音按钮
                const $audio_uk = $('<a>', { 
                    class: 'sound audio_play_button pron-uk icon-audio tts',
                    href: 'javascript:void(0)'
                }).appendTo(audioContainer);
                
                $audio_uk.on('click', (event) => {
                    event.stopPropagation();
                    event.preventDefault();
                    oaldpeInit.EdgeTTS.playText(exampleText, oaldpeConfig.onlineResources.britishTTS.selectedValue);
                });

                // 创建美音按钮
                const $audio_us = $('<a>', { 
                    class: 'sound audio_play_button pron-us icon-audio tts',
                    href: 'javascript:void(0)'
                }).appendTo(audioContainer);
                
                $audio_us.on('click', (event) => {
                    event.stopPropagation();
                    event.preventDefault();
                    oaldpeInit.EdgeTTS.playText(exampleText, oaldpeConfig.onlineResources.americanTTS.selectedValue);
                });

                // 将音频按钮容器插入到例句中
                const $xSpan = $example.find('.x, span.x, .unx, span.unx').first();
                if ($xSpan.length > 0) {
                    // 在 .x 或 .unx 元素的末尾插入
                    $xSpan.append(audioContainer);
                } else {
                    // 直接追加到 li
                    $example.append(audioContainer);
                }

                $example.attr('pron-uk', 'true');
                $example.attr('pron-us', 'true');

                console.log('[oaldpe] Example', index, 'text:', exampleText.substring(0, 30), '...');
            });

            console.log('[oaldpe] setupExamplePronTTS: Completed');
        }

                let inputText = $textContainer.clone()
                    .find('.gloss, chn, script, xt').remove().end().text()
                    .replace(/somebody\/something/g, 'somebody or something')
                    .replace(/&/g, 'and').replace(/\u200B/g, '');

                // 删除所有旧的音频按钮（.app, .app-ext, .tts 等）
                const $removed = $example.find('.audio_play_button').remove();
                console.log('[oaldpe] Example', index, 'removed', $removed.length, 'audio buttons');

                // 在 <xt> 标签之前插入音频按钮
                const $xtTag = $textContainer.find('xt');
                ...
                $audio_uk.on('click', (event) => {
                    event.stopPropagation();
                    event.preventDefault();
                    oaldpeInit.EdgeTTS.playText(inputText, oaldpeConfig.onlineResources.britishTTS.selectedValue);
                });
                ...
                $audio_us.on('click', (event) => {
                    event.stopPropagation();
                    event.preventDefault();
                    oaldpeInit.EdgeTTS.playText(inputText, oaldpeConfig.onlineResources.americanTTS.selectedValue);
                });

                if ($xtTag.length > 0) {
                    // 在 <xt> 之前插入音频按钮
                    $xtTag.before($audio_uk).before($audio_us);
                } else {
                    // 如果没有 <xt> 标签，插入到 <x> 或 <unx> 的最后
                    $textContainer.append($audio_uk).append($audio_us);
                }

                $example.attr('pron-uk', 'true');
                $example.attr('pron-us', 'true');
                
                console.log('[oaldpe] Example', index, 'added TTS audio buttons');
            });
        ...
        function setupUnboxFold() {
            $oaldpe.find('.collapse .unbox .box_title').each(function () {
                ...
                if (oaldpeConfig.collapseControl.unfoldUnbox.selectedValue || oaldpeConfig.collapseControl.autoUnfoldUnbox.selectedValue[$unbox.attr('unbox')]?.value) {
                    // DEBUG LOG
                    console.log('[Oxford Debug] Unbox expanding:', $unbox.attr('unbox'), 'Global Unfold:', oaldpeConfig.collapseControl.unfoldUnbox.selectedValue);
        ...
        function initCollapse() {
            ...
            $gear_icon.on('dblclick', function () {
                ...
                // if (\.attr("concise") === 'true') {
                //    \.removeAttr("concise");
                //    // 展开所有
                //    \.filter(".folded").trigger("click");
                //    \.filter((_, e) => \.parent(".shcut-g").hasClass("folded")).trigger("click");
                //    \.filter((_, e) => !\.parent().hasClass("expanded")).trigger("click");
                // } else {
                //    \.attr("concise", 'true');
                //    // 折叠所有
                //    \.not(".folded").trigger("click");
                //    \.filter((_, e) => \.parent(".unbox").hasClass("is-active")).trigger("click");
                //    \.filter((_, e) => \.parent().hasClass("expanded")).trigger("click");
                // }
